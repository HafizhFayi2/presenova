<?php
// ajax/load_attendance_form.php
session_start();
require_once '../includes/database_helper.php';
require_once '../includes/face_matcher.php';

$student_schedule_id = $_POST['student_schedule_id'] ?? null;
$subject = $_POST['subject'] ?? '';
$teacher = $_POST['teacher'] ?? '';

if (!$student_schedule_id) {
...
$sql = "
    ...
    AND ss.student_id = ?
";
$stmt = $db->query($sql, [$student_schedule_id, $_SESSION['student_id']]);
...
<div class="attendance-form-container">
    <div class="alert alert-info">
        <h5><i class="fas fa-book"></i> <?= htmlspecialchars($subject) ?></h5>
        <p class="mb-0"><i class="fas fa-chalkboard-teacher"></i> <?= htmlspecialchars($teacher) ?></p>
    </div>
    ...
    <div id="cameraSection" style="display: none;">
        ...
        <div class="text-center">
            <button id="captureAttendanceBtn" class="btn btn-primary" data-has-reference="<?= $hasReference ? '1' : '0' ?>" <?= !$hasReference ? 'disabled' : '' ?>>
    ...
    <input type="hidden" id="faceReferenceUrl" value="<?= htmlspecialchars($referenceUrl, ENT_QUOTES) ?>">
    ...
    <input type="hidden" id="studentScheduleId" value="<?= $student_schedule_id ?>">
</div>
...
$(document).ready(function() {
    ...
    const similarityThreshold = <?php echo defined('FACE_MATCH_THRESHOLD') ? (int) FACE_MATCH_THRESHOLD : 70; ?>;
    ...
    const studentLabel = <?php echo json_encode($student['student_name'] ?? $student['student_nisn'] ?? 'Siswa'); ?>;
