...
class DatabaseHelper {
    ...
    private function getFirstScheduleDate(DateTime $startDate, int $dayId): DateTime {
        ...
        $startDow = (int) $startDate->format('N'); // 1=Mon..7=Sun
    ...
    public function getStudentSchedule($student_id, $class_id) {
        ...
        // Hitung batas minggu (reset Sabtu 15:00 WIB)
        $tz = new DateTimeZone('Asia/Jakarta');
        $now = new DateTime('now', $tz);
        $weekStart = (clone $now)->modify('monday this week')->setTime(0, 0, 0);
        $resetTime = (clone $weekStart)->modify('+5 days')->setTime(15, 0, 0); // Sabtu 15:00
        if ($now >= $resetTime) {
    ...
    /**
     * Check if student can attend based on schedule time
     */
