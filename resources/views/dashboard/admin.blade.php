    ...
    switch ($table) {
        case 'student':
            ...
            if ($student_id === 0) {
                ...
                $success = "Siswa berhasil ditambahkan. Password default mengikuti NISN siswa.";
            } else {
...
<body>
    ...
    <div class="container-fluid p-0">
        ...
        <div class="main" id="main">
            ...
            <div id="content">
                ...
                if (in_array($table, $available_admin_sections, true)) {
                    echo view('dashboard.roles.admin.sections.' . $table, get_defined_vars())->render();
                } else {
                    // Default dashboard
                    // Get statistics
                    $stats = [];
    ...
    function prefetchAdminUrl(rawUrl) {
        const url = normalizeAdminPrefetchUrl(rawUrl);
        if (!url || adminPrefetchLoaded.has(url) || url === window.location.href) {
            return;
        }
        if (shouldSkipAdminPrefetch(url)) {
            return;
        }
        adminPrefetchLoaded.add(url);
        fetch(url, {
            method: 'GET',
            ...
            adminPrefetchLoaded.delete(url);
        });
    }

    function shouldSkipAdminPrefetch(url) {
        try {
    ...
    function initAdminSectionPrefetch() {
        const links = Array.from(document.querySelectorAll('.sidebar-menu a.nav-link[href], .quick-action-item[href]'));
        if (!links.length) {
            return;
        }
        ...
        const queue = [];
        links.forEach((link) => {
            const normalized = normalizeAdminPrefetchUrl(link.href);
            if (!normalized) {
                return;
            }
            queue.push(normalized);
            link.addEventListener('mouseenter', () => prefetchAdminUrl(normalized), { passive: true });
            link.addEventListener('focus', () => prefetchAdminUrl(normalized), { passive: true });
            link.addEventListener('touchstart', () => prefetchAdminUrl(normalized), { passive: true, once: true });
        });
        ...
        const prime = () => {
            queue
                ...
                .slice(0, 4)
                .forEach((url, index) => {
                    setTimeout(() => prefetchAdminUrl(url), index * 200);
                });
        };
