...
<script>
const siswaPrintConfiguredBase = <?php
$siswaPathBase = trim((string) request()->getBasePath(), '/');
if ($siswaPathBase === '') {
    $siswaPathBase = trim((string) parse_url((string) config('app.url'), PHP_URL_PATH), '/');
}
echo json_encode($siswaPathBase);
?>;

function resolveSiswaPrintPrefix() {
    ...
    if (siswaPrintConfiguredBase) {
        return '/' + String(siswaPrintConfiguredBase).replace(/^\/+|\/+$/g, '');
    }
    return '';
}

const siswaPrintPrefix = resolveSiswaPrintPrefix();

// Print schedule (global)
window.printSchedule = function() {
    const baseUrl = new URL(siswaPrintPrefix + '/dashboard/roles/siswa/print/jadwal_print.php', window.location.origin);
