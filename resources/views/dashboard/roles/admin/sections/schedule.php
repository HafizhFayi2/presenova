...
<div class="data-table-container">
    <div class="table-header">
        ...
        <div class="d-flex gap-2">
            <button type="button" class="btn btn-schedule-print" onclick="return openAdminSchedulePrintDialog();">
...
const adminPrintConfiguredBase = <?php
$adminPathBase = trim((string) request()->getBasePath(), '/');
if ($adminPathBase === '') {
    $adminPathBase = trim((string) parse_url((string) config('app.url'), PHP_URL_PATH), '/');
}
echo json_encode($adminPathBase);
?>;

function resolveAdminPrintPrefix() {
    ...
    if (adminPrintConfiguredBase) {
        return '/' + String(adminPrintConfiguredBase).replace(/^\/+|\/+$/g, '');
    }
    return '';
}

const adminPrintPrefix = resolveAdminPrintPrefix();
...
function openAdminSchedulePrintDialog() {
    ...
    const baseUrl = new URL(adminPrintPrefix + '/dashboard/roles/admin/print/jadwal_print.php', window.location.origin);
