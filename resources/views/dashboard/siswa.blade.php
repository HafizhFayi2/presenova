...
if (!function_exists('hasPoseCaptureDataset')) {
    function hasPoseCaptureDataset($studentNisn, $className, $studentName)
    {
        ...
        $baseDir = public_path('uploads/faces');
...
$storedStudentPassword = (string) ($student['student_password'] ?? '');
...
foreach ($defaultCandidates as $candidate) {
    $candidateHash = hash('sha256', $candidate . PASSWORD_SALT);
    if ($storedStudentPassword !== '' && hash_equals($storedStudentPassword, $candidateHash)) {
        $requiresPasswordChange = true;
        break;
    }
}

if ($requiresPasswordChange && $page !== 'change_password') {
    header('Location: siswa.php?page=change_password');
    exit();
}
...
<body data-enable-push="1">
    ...
    <div class="student-shell container-fluid">
        ...
        <div class="main-content" id="mainContent">
            ...
            <div id="content">
                ...
                switch($page) {
                    case 'jadwal':
                        echo view('dashboard.roles.siswa.sections.jadwal', get_defined_vars())->render();
                        break;
                    case 'face_recognition':
                        echo view('dashboard.roles.siswa.sections.face_recognition', get_defined_vars())->render();
                        break;
                    case 'riwayat':
                        echo view('dashboard.roles.siswa.sections.riwayat', get_defined_vars())->render();
                        break;
                    case 'profil':
                        echo view('dashboard.roles.siswa.sections.profil', get_defined_vars())->render();
                        break;
                    case 'change_password':
                        echo view('dashboard.roles.siswa.sections.change_password', get_defined_vars())->render();
                        break;
                    default: // dashboard
                        echo view('dashboard.roles.siswa.sections.dashboard', get_defined_vars())->render();
                }
                ?>
            </div>
        </div>
    ...
    const sectionPrefetchLoaded = new Set();
    ...
    function prefetchSectionUrl(rawUrl) {
        const url = normalizePrefetchUrl(rawUrl);
        if (!url || sectionPrefetchLoaded.has(url) || url === window.location.href) {
            return;
        }
        if (shouldSkipStudentPrefetch(url)) {
            return;
        }
        sectionPrefetchLoaded.add(url);
        fetch(url, {
            method: 'GET',
            ...
            sectionPrefetchLoaded.delete(url);
        });
    }

    function shouldSkipStudentPrefetch(url) {
        try {
    ...
    function initStudentSectionPrefetch() {
        const sectionLinks = Array.from(document.querySelectorAll('.sidebar-menu a.nav-link[href]'));
        ...
        sectionLinks.forEach((link) => {
            const normalized = normalizePrefetchUrl(link.href);
            if (!normalized) {
                return;
            }
        ...
        const primePrefetch = () => {
            queuedUrls
                ...
                .slice(0, 3)
                .forEach((url, index) => {
                    setTimeout(() => prefetchSectionUrl(url), index * 220);
                });
        };

        if ('requestIdleCallback' in window) {
            window.requestIdleCallback(primePrefetch, { timeout: 1800 });
            return;
        }
        setTimeout(primePrefetch, 650);
    }
